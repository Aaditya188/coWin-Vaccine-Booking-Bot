# coWin Vaccine Booking Bot
Easy Vaccine Booking Facility During this Pandemic 

# Submission by Team NEXUS for HackACode 2021

<h2 align= "left"><b>Our Product</b></h2>

We NEXUS present before you,<b>"coWin Vaccine Booking Bot"</b>.

<img width=40% src="Screenshots/logo.jpeg"> &ensp;


## Our Idea
No more waiting hours to find a slot for vaccination. With the help of our CoWin bot, you can fill in the details and wait for the bot to find free slot on its own.

## Problems we Faced :
We faced many problems in importing the API and launching the webpage on our domain. It took a while but we finlly achieved our goal.
 
# WHAT YOU WILL EXPERIENCE ?
Easy, Fast and Hastle Free Vaccine Slot Booking

<h2 align= "left"><b>Website in a Print view</b></h2>

<p align="left">  

<img width=40% src="Screenshots/1.JPG"> &ensp;
<img width=40% src="Screenshots/2.JPG"> &ensp;
<img width=40% src="Screenshots/3.JPG"> &ensp;
<img width=40% src="Screenshots/4.JPG"> &ensp;
<img width=40% src="Screenshots/5.JPG"> &ensp;
<img width=40% src="Screenshots/6.JPG"> &ensp;
<img width=40% src="Screenshots/7.JPG"> &ensp;
<img width=40% src="Screenshots/8.JPG"> &ensp;
<img width=40% src="Screenshots/9.JPG"> &ensp;
<img width=40% src="Screenshots/10.JPG"> &ensp;
<img width=40% src="Screenshots/11.JPG"> &ensp;
<img width=40% src="Screenshots/12.JPG"> &ensp;

## Tech Stack

- eJS, CSS, Bootstrap, HTML, JS, JSON, JQuery
  
 
<h2 align= "left"><b>Demo Video and Website Link 👇</b></h2>

- Website Link : <a href="https://www.cowinbot.nexustech.studio/" target="_blank">book your Slot Now</a>
- Youtube Demo : <a href="">Click to watch full video</a>
- PPT Link : <a href="https://www.canva.com/design/DAEmfTOhy5s/K3Sug0xqGvt2ji4OmH_YPw/watch?utm_content=DAEmfTOhy5s&utm_campaign=designshare&utm_medium=link&utm_source=publishsharelink">PPT is Here</a>
